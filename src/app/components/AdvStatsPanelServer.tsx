...
export default async function AdvStatsPanelServer() {
  // ✅ Use the LEAGUE sheet (primary), fall back only if truly missing
  const spreadsheetId =
  ...
  // Find the exact tab title for ADV STATS by gid (from your link)
  ...
  const allProps =
    meta.data.sheets?.map((sh) => sh.properties!).filter(Boolean) ?? [];

  // Prefer lookup by gid (sheetId)
  let advProps = allProps.find((p) => p.sheetId === ADV_GID);

  // Fallback: fuzzy title match in case gid changed (remove spaces, lowercase)
  if (!advProps) {
    advProps = allProps.find(
      (p) => (p.title ?? "").replace(/\s+/g, "").toLowerCase() === "advstats"
    );
  }

  if (!advProps?.title) {
    const titles = allProps.map((p) => p.title).filter(Boolean).join(", ");
    return (
      <div className="p-6 rounded-2xl bg-zinc-900/70 border border-zinc-800 text-zinc-300">
        Could not locate the <b>ADV STATS</b> tab in the LEAGUE sheet. Known tabs:
        {" "}
        <code>{titles || "(none found)"}</code>
        <div className="mt-2 text-sm text-zinc-400">
          Ensure the data lives in the spreadsheet set by <code>NCX_LEAGUE_SHEET_ID</code>.
        </div>
      </div>
    );
  }
  ...
  const t1Rows: Table1Row[] = (t1Res.data.values ?? [])
    .map((r) => ({
      team: s(r[0]),
      totalGames: s(r[1]),
      avgWins: s(r[2]),
  ...
  const t2Rows: Table2Row[] = (t2Res.data.values ?? [])
    .map((r) => ({
      scenario: s(r[0]),
      avgHomePts: s(r[1]),
      avgAwayPts: s(r[2]),
  ...
  const t3Rows: Table3Row[] = (t3Res.data.values ?? [])
    .map((r) => ({
      scenario: s(r[0]),
      republic: s(r[1]),
      cis: s(r[2]),
  ...
  const t4Rows: Table4Row[] = (t4Res.data.values ?? [])
    .map((r) => ({
      factionVs: s(r[0]),
      republic: s(r[1]),
      cis: s(r[2]),
  ...
  const t5Rows: Table5Row[] = (t5Res.data.values ?? [])
    .map((r) => ({
      faction: s(r[0]),
      wins: s(r[1]),
      losses: s(r[2]),
